...
Under the same bias-load test, steady-state tilt is eliminated ($\theta_{ss} \approx 0^\circ$). The cost: settling increases to $5.29$ s and peak voltage rises to $11.06$ V.
...
\textcolor{red}{TODO: build the practical controller using measured }$x_c$\textcolor{red}{ and }$\theta$\textcolor{red}{, estimate }$\dot{x}_c$\textcolor{red}{ and }$\dot{\theta}$\textcolor{red}{ with filtered derivatives, and validate the balance controller against the official disturbance test before deployment.}
