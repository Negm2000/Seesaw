...
The controller stabilizes the system and minimizes the steady-state limit cycle caused by friction and quantization. Asymptotic convergence is precluded by these nonlinearities.

\subsubsection{Pole Selection Rationale}

The gain vector $\mathbf{K}$ is computed by placing the closed-loop poles $\{p_1,p_2,p_3,p_4\}$ to satisfy performance requirements. Complex poles are characterized by decay rate $\sigma$ and damping ratio $\zeta$ to map directly to settling time ($t_s \approx 4/\sigma$) and overshoot ($M_p = f(\zeta)$). Subscript ``$th$'' denotes the dominant balancing modes.

\paragraph{Pole roles.} For this 4-state plant we use:
\begin{itemize}
	\item $p_{1,2}$: a complex pair governing the $\theta$ dynamics (the unstable balancing mode);
...
\begin{itemize}
	\item \textbf{Decay rate $\sigma_{th}$:} Set to $4.5$ rad/s to provide a 2x stability margin over the unstable open-loop pole $p_{OL} \approx 2.24$ rad/s.
	\item \textbf{Damping $\zeta_{th}$:} Set to $0.8$ to ensure a transient overshoot of $1.5\%$ while maintaining fast recovery.
\end{itemize}

\paragraph{$p_3, p_4$: Non-dominant modes.} These poles are placed further into the left half-plane than $p_{1,2}$ to ensure the balancing pair remains the dominant system response.
\begin{itemize}
	\item $p_3 = -6.75$ rad/s: Damps the cart velocity transient.
	\item $p_4 = -5.4$ rad/s: Regulates the cart return-to-center.
\end{itemize}
...
\begin{figure}[H]
	...
	\caption{Closed-loop poles. The dominant balancing pair sits at $-\sigma_{th} \pm j\omega_{d,th}$, closest to the imaginary axis. Both cart poles ($p_3$, $p_4$) are further from the jw-axis, ensuring the complex pair genuinely governs the system's overall settling time.}
	\label{fig:cl_poles_final}
\end{figure}

The initial condition test uses $\theta(0) = 2^\circ$ (Fig.~\ref{fig:ic_final}). This represents a realistic hardware startup configuration, accounting for the limit-cycle behavior and manual alignment precision. Peak values from the linear simulation:
\begin{equation*}
	V_m = 3.87\;\text{V}, \qquad \theta = 2.55^\circ, \qquad x_c = 3.26\;\text{cm}.
\end{equation*}
All well inside $\pm 6$ V, $\pm 11.5^\circ$. Peak voltage scales linearly with IC, so the linear envelope reaches $\theta_{\max} = 3.10^\circ$ before $V_m$ hits saturation. Beyond that the saturation block clips $V_m$, the placed pole locations no longer hold, and the controller still recovers but with degraded margins. The $3.10^\circ$ figure bounds the linear regime, not the recovery capability.
...
An off-center mass (100 g) induces a constant torque $\tau_{bias} \approx 0.123$ Nm. Without integral action, the system settles at a steady-state offset $\theta_{ss}$ (Fig.~\ref{fig:rep_final}). On hardware, this shifts the center of the limit-cycle oscillation.
...
\end{equation}
An integrator pole is placed at $p_{int} = -0.5$ rad/s. Integral action restores the oscillation center to $\theta=0$ but does not reduce the limit-cycle amplitude.
...
At $k_{\mathrm{obs}} = 2.0$, the $\theta$-channel noise gain is $0.055$ V/count, matching the full-state baseline. The observer bandwidth ($\approx 9$ rad/s) is three orders of magnitude below the sampling rate (1 kHz), providing inherent low-pass filtering of encoder quantization noise. This provides superior phase margin compared to a differentiator-LPF scheme for the same noise budget.
