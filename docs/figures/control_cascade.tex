...
\begin{tikzpicture}
  ...
  \begin{scope}[on background layer]
    ...
    %% ── Outer loop (Cascaded Control Boundary) ──────────────────────────────
    %% Increase padding to avoid intersecting signal labels and nodes
    \draw[dash pattern=on 4pt off 3pt, gray!30, rounded corners=12pt, line width=0.5pt]
      let \p1 = (So.north west), \p2 = (tap), \p3 = (fbX)
      in (\x1-22pt, \y3-25pt) rectangle (\x2+24pt, \y1+24pt);

    \node[gray!50, font=\small\itshape, anchor=south west]
      at ($(So.north west)+(-19pt, 24pt)$) {Outer loop};

    %% ── Inner loop (Stability Loop Boundary) ────────────────────────────────
    \draw[dash pattern=on 3pt off 3pt, blue!22, rounded corners=10pt, line width=0.5pt]
      let \p1 = (Si.north west), \p2 = (tap), \p3 = (fbA)
      in (\x1-15pt, \y3-16pt) rectangle (\x2+15pt, \y1+16pt);

    \path let \p1 = (Si.north west), \p3 = (fbA)
      in node[blue!42, font=\small\itshape, anchor=south west]
         at (\x1-12pt, \y3-14pt) {Inner loop};
