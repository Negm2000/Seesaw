...
%    Phase 2: A_sw,   B_sw,   C_sw,   D_sw     (states: x_c, alpha, x_c_dot, alpha_dot)
%
...
%  States:  [x_c; alpha; x_c_dot; alpha_dot]
%  =====================================================================
mdl2 = 'Seesaw_Full';
fprintf('\n--- Building Phase 2: %s ---\n', mdl2);
...
% Input:  V_m (1 signal)
% Output: [x_c; alpha; x_c_dot; alpha_dot] (full state for simulation scopes)
...
% ---- DEMUX: split [x_c; alpha; x_c_dot; alpha_dot] ----
...
% Port 3 = x_c_dot --> Scope + ToWS
add_line(mdl2, 'Demux_SS/3', 'Cart Velocity/1', 'autorouting', 'smart');
add_line(mdl2, 'Demux_SS/3', 'ToWS_xdot/1',    'autorouting', 'smart');

% Port 2 = alpha --> rad_to_deg --> Scope + ToWS
add_line(mdl2, 'Demux_SS/2', 'rad_to_deg/1',        'autorouting', 'smart');
add_line(mdl2, 'rad_to_deg/1', 'Seesaw Angle [deg]/1', 'autorouting', 'smart');
add_line(mdl2, 'Demux_SS/2', 'ToWS_alpha/1',        'autorouting', 'smart');
...
%  States:  [x_c; alpha; x_c_dot; alpha_dot]   (same as Phase 2)
%
...
if ~exist(ctrl_path, 'file')
    ...
    % Push validation-order gains [x_c, x_c_dot, alpha, alpha_dot] to base.
    if isfield(ctrl, 'K')
        K_fb = ctrl.K;
    elseif isfield(ctrl, 'Kf')
        K_fb = ctrl.Kf;
    else
        error('controller_freq.mat must contain K or Kf.');
    end
    ...
    if quarc_available
        ...
        % State-Space plant. C_sw exposes measured [x_c; alpha], so use
        % eye(4) here to route full simulation state feedback.
        % Initial condition: tilted ~4.5° to test disturbance rejection
        add_block('simulink/Continuous/State-Space', [mdl3 '/Plant_SS'], ...
            'A', 'A_sw', 'B', 'B_sw', 'C', 'eye(4)', 'D', 'zeros(4,1)', ...
            'X0', '[0; 0.08; 0; 0]', ...
        ...
        % x_c_dot (port 3) -> Mux port 2
        add_line(mdl3, 'Demux_Plant/3', 'Mux_states/2', 'autorouting', 'smart');
        % alpha (port 2) -> Mux port 3
        add_line(mdl3, 'Demux_Plant/2', 'Mux_states/3', 'autorouting', 'smart');
        ...
        add_line(mdl3, 'xc_m_to_cm/1',  'Cart [cm]/2',    'autorouting', 'smart');
        add_line(mdl3, 'Demux_Plant/2', 'alpha_to_deg/1',  'autorouting', 'smart');
        add_line(mdl3, 'alpha_to_deg/1', 'Angle [deg]/1',  'autorouting', 'smart');

        % ---- Feedback → To Workspace ----
        add_line(mdl3, 'Demux_Plant/1', 'ToWS_xc/1',    'autorouting', 'smart');
        add_line(mdl3, 'Demux_Plant/2', 'ToWS_alpha/1', 'autorouting', 'smart');
...
fprintf('   Sine --> Amp --> Sat --> SS(4x4) --> [x_c, alpha, x_c_dot, alpha_dot]\n\n');
