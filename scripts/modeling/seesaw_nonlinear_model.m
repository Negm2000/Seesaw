...
data_file = fullfile(SEESAW_ROOT, 'data', 'cartModeling', 'nonlinear_output.mat');
...
nonlinear_input = [t_hw(:)'; 1.2*V_cmd_hw(:)'];
...
ud_sym_values = [ud_pos, ud_neg];
ud_sym_values = ud_sym_values(isfinite(ud_sym_values));
if isempty(ud_sym_values)
    ud_sym = NaN;
else
    ud_sym = mean(ud_sym_values);
end
