...
impulse_angle = deg2rad(target_angle_deg);
theta_ref = impulse_angle * impulse(F_bessel_out, t);
...
subplot(2,2,1);
plot(t, rad2deg(theta_rad), 'r--', 'LineWidth', 1.5); hold on;
plot(t, rad2deg(theta_ref), 'b', 'LineWidth', 1); grid on;
title('Pendulum Angle ($\theta$)'); ylabel('Degrees');
legend('Simulated Angle', 'Reference ($\theta_{ref}$)');
yline(0, 'k:', 'HandleVisibility', 'off');
yline(THETA_LIMIT_DEG, 'k--', 'HandleVisibility', 'off'); yline(-THETA_LIMIT_DEG, 'k--', 'HandleVisibility', 'off');

% 2. Cart Position
subplot(2,2,2);
plot(t, x_pos, 'g--', 'LineWidth', 1.5); hold on;
plot(t, x_ref, 'b', 'LineWidth', 1); grid on;
...
subplot(2,2,[3 4]);
plot(t, v_cmd, 'b', 'LineWidth', 1.5); grid on;
