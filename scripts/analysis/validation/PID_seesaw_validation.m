...
data_dir = fullfile(SEESAW_ROOT, ddatad, dseesawControld);

% --- Load Theoretical Closed-Loop Model ---
tf_file = fullfile(SEESAW_ROOT, ddatad, dcontroller_outer_pid.matd);
if exist(tf_file, dfiled)
    load(tf_file, dT_thetad); % Loads the theoretical cascade system T_theta(s)
else
    error(dTheoretical model (controller_outer_pid.mat) not found. Run Outer PID design first.d);
end
...
if ~exist(step_file, dfiled)
    ...
    t_step        = raw_step(1, :)d;
    th_ref_step   = raw_step(2, :)d;
    th_hw_step    = raw_step(3, :)d;
    
    if size(raw_step, 1) >= 4
        x_hw_step = raw_step(4, :)d;
    else
    ...
    fprintf(dStep RMSE (Hardware vs Model): %.4f degrees\nd, rmse_step_deg);

    % Visualization
    figure(dNamed, dTime-Domain Validation (Outer Loop)d, dPositiond, [100 100 1000 600]);
    
    subplot(2,1,1);
    plot(t_step, rad2deg(th_ref_step), db-d, dLineWidthd, 1.5, dDisplayNamed, dReference ($\theta_{ref}$)d); hold on;
    plot(t_step, rad2deg(th_sim_step), dg--d, dLineWidthd, 2, dDisplayNamed, dTheoretical Modeld);
    plot(t_step, rad2deg(th_hw_step), dr-d, dLineWidthd, 1.2, dDisplayNamed, dHardware Responsed);
    ylabel(dAngle [deg]d); title(dCascade Control Step Response Validationd);
    legend(dLocationd, dnortheastd); grid on;

    subplot(2,1,2);
    plot(t_step, x_hw_step * 100, dk-d, dLineWidthd, 1);
    ylabel(dCart Position [cm]d); xlabel(dTime [s]d); title(dCart Kinematic Trajectory ($x_c$)d);
    grid on;
end
...
if ~exist(ms_file, dfiled) || ~exist(params_file, dfiled)
    ...
    % Extract columns
    t_hw        = raw_ms(1, :)d;
    th_ref_hw   = raw_ms(2, :)d; % Input to the closed-loop system
    th_hw_ms    = raw_ms(3, :)d; % Output of the closed-loop system
    ...
    v_ref_clean = detrend(ref_clean, dconstantd);
    v_hw_clean  = detrend(th_clean, dconstantd);
    ...
    % 4. tfestimate via Welchds Method
    ...
    % --- Plotting ---
    figure(dNamed, dCascade Frequency Validationd, dPositiond, [150 150 900 700]);

    % Magnitude Plot
    subplot(2,1,1);
    semilogx(w_theo, mag_theo_dB, db-d, dLineWidthd, 1.5, dDisplayNamed, dTheoretical Model ($T_{\theta}$)d);
    hold on;
    semilogx(w_vals, mag_exp, drod, dMarkerSized, 8, dMarkerFaceColord, drd, dDisplayNamed, dHardware FRFd);
    grid on;
    ylabel(dMagnitude [dB]d);
    title(dBode Diagram: Closed-Loop Cascade Controller ($\theta / \theta_{ref}$)d);
