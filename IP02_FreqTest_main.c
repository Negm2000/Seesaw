...
typedef RT_MODEL_IP02_FreqTest_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * IP02_FreqTest(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(IP02_FreqTest_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://IP02_FreqTest:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: IP02_FreqTest -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
